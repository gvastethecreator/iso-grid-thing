...
import { Grid3x3, Square, MoveHorizontal, MoveVertical, Milestone, Orbit, Scaling } from 'lucide-react';
...
const PROJECTION_PRESETS: Record<string, number> = {
    'Isometric': 50, 'Military': 100, 'Cavalier': 75,
};
...
const GridSettingsPanel: React.FC<GridSettingsPanelProps> = ({ params, onParamChange, onParamChangeEphemeral, onInteractionStart }) => {
    ...
    const baseButtonClass = "tactile-surface py-1.5 px-2 rounded-sm text-[10px] font-bold uppercase tracking-wider text-zinc-400 hover:text-zinc-100 flex justify-center items-center";
    const toolButtonActive = "active text-indigo-400 !border-t-black/20 !border-b-white/10 !bg-zinc-950";
    ...
    return (
        <FloatingPanel title="Grid Layout" icon={<Grid3x3 size={15} />}>
            ...
            <CollapsibleSection title="Dimensions" icon={<Square size={12} className="text-zinc-500"/>} label="Dimensions" defaultOpen>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        >
                            {ASPECT_RATIOS.map(category => (
                                <optgroup key={category.group} label={category.group} className="bg-zinc-900 text-zinc-500">
                                    {category.options.map(opt => (
                                        <option key={opt} value={opt} className="text-zinc-300">{opt}</option>
                                    ))}
                                </optgroup>
                            ))}
                        </select>
            ...
            {!is2D && (
                <CollapsibleSection title="Projection" icon={<Milestone size={12} className="text-zinc-500"/>} label="Projection" defaultOpen>
                    <div className="grid grid-cols-3 gap-2 mb-3">
                    {Object.entries(PROJECTION_PRESETS).map(([name, value]) => (
                        <button key={name} onClick={() => handlePresetClick({ projectionAngle: value })}
