...
export default function AssetTimeline({ params, onParamsChange, selectedAssetId, onAssetSelect }: AssetTimelineProps) {
    ...
    return (
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-20 w-auto max-w-2xl bg-zinc-950/90 border border-white/5 rounded-xl p-2 flex gap-3 overflow-x-auto no-scrollbar shadow-[0_10px_40px_rgba(0,0,0,0.5)] backdrop-blur-md">
            <input 
                type="file" 
                ref={fileInputRef} 
                className="hidden" 
            ...
            {(params.assets || []).map(asset => (
                ...
                >
                    {asset.type === 'image' ? (
                        <img src={asset.src} alt="" className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110" />
                    ) : (
                        <>
                            <video src={asset.src} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110" />
