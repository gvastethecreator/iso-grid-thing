...
import React, { useRef, useEffect, forwardRef, useState, useMemo, useImperativeHandle, memo, useCallback } from 'react';
...
const IsometricGrid = forwardRef<SVGSVGElement, IsometricGridProps>((props, ref) => {
  ...
  // Smooth Camera State
  const [camera, setCamera] = useState({ rot: props.gridRotation || 0, proj: props.projectionAngle || 50 });
  const cameraProxy = useRef({ rot: props.gridRotation || 0, proj: props.projectionAngle || 50 });

  useEffect(() => {
      if (typeof gsap === 'undefined') return;
      gsap.to(cameraProxy.current, {
  ...
  const projection = useMemo(() => createGridProjection({
    width: props.width,
    depth: props.depth,
    ...
    containerSize,
    camera,
  }), [props.width, props.depth, props.padding, props.viewMode, props.gridGap, props.assets, containerSize, camera]);

  const fromScreenPoint = useCallback((screenX: number, screenY: number) => {
  ...
  useEffect(() => {
    ...
    gsap.to(groupRef.current, { attr: { transform: `translate(${translateX}, ${translateY})` }, duration: ANIM_DURATION, ease: ANIM_EASE });
    gsap.to(internalSvgRef.current, { backgroundColor: props.backgroundColor, duration: ANIM_DURATION, ease: ANIM_EASE });
  ...
  useEffect(() => {
    ...
    const { backgroundGrid, lineColor, lineThickness, width, depth, gridGap, lineStyle = 'solid' } = props;
    const strokeStyle = getStrokeStyle(lineStyle, lineThickness);
    
    gsap.to(bgGridGroupRef.current, { opacity: (props.lineOpacity ?? 1) * (backgroundGrid.enabled ? backgroundGrid.opacity : 0), duration: ANIM_DURATION, ease: ANIM_EASE });
    gsap.set(bgGridGroupRef.current, { attr: { stroke: lineColor, 'stroke-width': lineThickness, 'stroke-dasharray': strokeStyle.dasharray, 'stroke-linecap': strokeStyle.linecap } });
    ...
    if (bgGridGroupRef.current.children[0]) gsap.set(bgGridGroupRef.current.children[0], { attr: { d: paths.primary } });
    if (bgGridGroupRef.current.children[1]) gsap.set(bgGridGroupRef.current.children[1], { attr: { d: paths.secondary } });
  }, [props.backgroundGrid, props.lineColor, props.lineThickness, props.lineStyle, props.width, props.depth, props.gridGap, props.lineOpacity, projection]);
  ...
  }, [props.fillColor, props.fillEnabled, props.fillOpacity, props.width, props.depth, props.gridGap, projection]);
  ...
  useEffect(() => {
    ...
    gsap.to(mainGridLinesGroupRef.current, { opacity: props.lineOpacity ?? 1, duration: ANIM_DURATION, ease: ANIM_EASE });
    gsap.set(mainGridLinesGroupRef.current, { attr: { stroke: lineColor, 'stroke-width': lineThickness, 'stroke-dasharray': strokeStyle.dasharray, 'stroke-linecap': strokeStyle.linecap } });

    const paths = buildMainGridLinePaths({ width, depth, gridGap: gridGap || 0, projection });

    if (mainGridLinesGroupRef.current.children[0]) gsap.set(mainGridLinesGroupRef.current.children[0], { attr: { d: paths.primary } });
    if (mainGridLinesGroupRef.current.children[1]) gsap.set(mainGridLinesGroupRef.current.children[1], { attr: { d: paths.secondary } });
  }, [props.lineColor, props.lineOpacity, props.lineThickness, props.lineStyle, props.width, props.depth, props.gridGap, projection]);
  ...
  useEffect(() => {
    ...
    (props.assets || []).forEach(asset => {
        ...
        const elWrap = document.querySelector(`g[data-id="${asset.id}"]`);
        if (elWrap) {
             gsap.set(elWrap, { attr: { transform: `translate(${startPos.x}, ${startPos.y})` } });
        }
        
        const elMatrix = document.querySelector(`g[data-matrix-id="${asset.id}"]`);
        if (elMatrix) {
             gsap.set(elMatrix, { attr: { transform: `matrix(${matrix.a}, ${matrix.b}, ${matrix.c}, ${matrix.d}, 0, 0)` } });
        }
    });
  }, [props.assets, props.gridGap, projection]);

  const handleDragMouseDown = (type: 'asset', id: string, e: React.MouseEvent) => {
    ...
    // Calculate initial mouse offset in relative grid coordinates
    ...
    dragState.current = { type, id, element, finalGridPos: initialGridPos, moved: false, dragOffset };
    document.body.style.cursor = 'grabbing';
  };

  const handleSvgClick = (e: React.MouseEvent<SVGSVGElement>) => {
    if ((e.target as SVGElement).closest('.interactive-grid-element')) return;
    if (props.selectedAssetId) props.onAssetSelect(null);
  }
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (dragState.current) {
        ...
        const newGridPos = fromScreenPoint(e.clientX, e.clientY);
        let gridX = Math.round(newGridPos.x + (dragOffset?.x || 0));
        let gridY = Math.round(newGridPos.y + (dragOffset?.y || 0));

        if (type === 'asset') {
          const a_asset = (props.assets || []).find(ass => ass.id === id);
          if (a_asset) {
              const aw = Math.ceil(a_asset.width);
              const ad = Math.ceil(a_asset.depth);
              gridX = Math.max(0, Math.min(props.width - aw, gridX));
              gridY = Math.max(0, Math.min(props.depth - ad, gridY));
              dragState.current.finalGridPos = { x: gridX, y: gridY };
              
              const gap = props.gridGap || 0;
              const startPos = projection.toIso(gridX * (1 + gap), gridY * (1 + gap));
              gsap.to(element, { attr: { transform: `translate(${startPos.x}, ${startPos.y})` }, duration: 0, ease: 'none' });
          }
        }
      }
    };
    ...
    const handleMouseUp = () => {
      ...
      if (dragState.current) {
        ...
        if (moved) {
          if (type === 'asset') {
              const updatedAssets = (props.assets || []).map(a => a.id === id ? { ...a, x: finalGridPos.x, y: finalGridPos.y } : a);
              if (props.onParamsChangeComplete) {
                  props.onParamsChangeComplete({ assets: updatedAssets });
              } else {
                  props.onParamsChange({ assets: updatedAssets });
              }
          }
        }
        setTimeout(() => { if (dragState.current) dragState.current.moved = false; }, 0);
  ...
  }, [fromScreenPoint, projection, props, getCursor]);

    const handleContainerMouseMove = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
        // Panning
        if (panState.current.isPanning) {
            viewStateRef.current.offset = { x: e.clientX - panState.current.start.x, y: e.clientY - panState.current.start.y };
            gsap.set(viewGroupRef.current, {
                attr: { transform: `translate(${viewStateRef.current.offset.x}, ${viewStateRef.current.offset.y}) scale(${viewStateRef.current.zoom})` }
            });
        ...
        if (props.activeTool === 'paint' || props.activeTool === 'text' || dragState.current) {
            ...
            if (dragState.current) {
                ...
                if (dragState.current.type === 'asset') {
                    ...
                    if (a) { targetW = a.width; targetD = a.depth; }
                }
            }
            ...
            let attrs = { d, fill: 'none', stroke: '#a1a1aa', 'stroke-dasharray': '4', 'stroke-width': 1.5 / viewStateRef.current.zoom, opacity: 1, visibility: 'visible' };
            gsap.set(previewGroupRef.current?.children[0], { attr: attrs });
            ...
            gsap.set(previewGroupRef.current?.children[1], { attr: { d: guidesD, stroke: '#ec4899', 'stroke-dasharray': '4', 'stroke-width': 1 / viewStateRef.current.zoom, fill: 'none', opacity: 0.5, visibility: 'visible' }});
            
        } else {
            gsap.set(previewGroupRef.current?.children[0], { attr: { visibility: 'hidden' } });
            gsap.set(previewGroupRef.current?.children[1], { attr: { visibility: 'hidden' } });
        }
    ...
    const handlePanMouseDown = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
        ...
        // Handle Pan
        panState.current = { isPanning: true, start: { x: e.clientX - viewStateRef.current.offset.x, y: e.clientY - viewStateRef.current.offset.y } };
    ...
    const handleWheel = useCallback((e: React.WheelEvent<HTMLDivElement>) => {
        ...
        const newZoom = e.deltaY < 0 ? viewStateRef.current.zoom * zoomFactor : viewStateRef.current.zoom / zoomFactor;
        const clampedZoom = Math.min(Math.max(newZoom, 0.1), 10); // Limit zoom
        const zoomRatio = clampedZoom / viewStateRef.current.zoom;

        const newOffsetX = mouseX - (mouseX - viewStateRef.current.offset.x) * zoomRatio;
        const newOffsetY = mouseY - (mouseY - viewStateRef.current.offset.y) * zoomRatio;
  ...
  return (
    <div ref={containerRef} className="w-full h-full overflow-hidden relative"
        ...
        onWheel={handleWheel}
    >
      <svg ref={internalSvgRef} width="100%" height="100%" style={{ backgroundColor: props.backgroundColor }} onClick={handleSvgClick}>
        <g ref={viewGroupRef}>
            <g ref={groupRef}>
              <g ref={bgGridGroupRef} strokeLinecap="round" strokeLinejoin="round" fill="none" vectorEffect="non-scaling-stroke">
                  <path key="bg-y-lines" />
                  <path key="bg-x-lines" />
              </g>
              ...
              <g ref={mainGridLinesGroupRef} strokeLinecap="round" strokeLinejoin="round" fill="none" vectorEffect="non-scaling-stroke">
                  <path key="main-y-lines" />
                  <path key="main-x-lines" />
              </g>

              <g id="selection-highlight-group" ref={selectionHighlightGroupRef} style={{ pointerEvents: 'none' }}>
                <path style={{ visibility: 'hidden' }} fill="none" stroke="#6366f1" strokeWidth={3 / viewStateRef.current.zoom} strokeDasharray={`${6 / viewStateRef.current.zoom}`} vectorEffect="non-scaling-stroke"/>
              </g>

              {/* Assets Group */}
              <g id="assets-group">
                {[...(props.assets || [])].sort((a,b) => (a.x + a.y) - (b.x + b.y)).map(asset => {
                  const gap = props.gridGap || 0;
                  ...
                  return (
                    <g key={asset.id} 
                       ...
                       onMouseDown={(e) => { e.stopPropagation(); handleDragMouseDown('asset', asset.id, e); }}
                       style={{ cursor: 'grab', pointerEvents: 'bounding-box' }}>
                       
                       <defs>
                          <clipPath id={`clip-${asset.id}`}>
                             <rect x="0" y="0" width={imgWUnscaled} height={imgHUnscaled} rx={rx} ry={rx} />
                          </clipPath>
                       </defs>

                       <g data-matrix-id={asset.id}>
                          {rx > 0 && <rect x="0" y="0" width={imgWUnscaled} height={imgHUnscaled} fill={props.backgroundColor} stroke="none" />}

                          <g clipPath={`url(#clip-${asset.id})`}>
                             {(props.fillEnabled ?? true) && (
                                 <rect x="0" y="0" width={imgWUnscaled} height={imgHUnscaled} 
                                       fill={props.fillColor} style={{ opacity: props.fillOpacity ?? 1 }} stroke="none" />
                             )}
                             <rect x="0" y="0" width={imgWUnscaled} height={imgHUnscaled} 
                                   fill="none" 
                             ...
                             <g transform={innerTransform}>
                               {asset.type === 'image' ? (
                                   <image 
                                      ...
                                      width={imgW} 
                                      height={imgH} 
                                      preserveAspectRatio={asset.objectFit === 'contain' ? 'xMidYMid meet' : 'xMidYMid slice'} 
                                      style={{ pointerEvents: 'none' }} 
                                   />
                               ) : (
                                   <foreignObject width={imgW} height={imgH} style={{ pointerEvents: 'none' }}>
                                     <video src={asset.src} autoPlay loop muted playsInline style={{ width: '100%', height: '100%', objectFit: asset.objectFit === 'contain' ? 'contain' : 'cover', pointerEvents: 'none' }} />
                                   </foreignObject>
                               )}
                             </g>
                          </g>
                          ...
                          <rect x="0" y="0" width={imgWUnscaled} height={imgHUnscaled} rx={rx} ry={rx} fill="transparent" style={{ pointerEvents: 'all' }} />
                       </g>
                    </g>
                  );
                })}
