  ...
  return (
    <div className="mb-3">
      <div className="flex justify-between items-center mb-1">
        <label
          htmlFor={inputId}
          className="flex items-center gap-2 text-xs font-semibold text-zinc-400 select-none"
        >
          {icon} {label}
        </label>
...
export const ControlColor: React.FC<ColorProps> = ({
  label,
  icon,
  value,
  onChange,
  ...
  useEffect(() => {
    ...
    const handleEscape = (event: KeyboardEvent) => {
      if (event.key === "Escape") setDisplayPicker(false);
    };
        ...
        <div style={popover}>
          ...
          <div className="w-56 bg-zinc-900 p-3 rounded-sm border border-white/10 shadow-2xl relative z-50">
            ...
            {(["r", "g", "b"] as const).map((channel) => (
              <label
                key={channel}
                className="grid grid-cols-[14px_1fr_32px] items-center gap-2 text-[10px] font-mono uppercase text-zinc-500 mb-2"
              >
                <span>{channel}</span>
                <input
                  type="range"
            ...
            <div
              className="mt-3 h-8 rounded-sm border border-black/40 shadow-inner"
...
export const Toggle: React.FC<ToggleProps> = ({ label, icon, checked, onChange }) => (
  <label className="flex items-center justify-between cursor-pointer mb-2 select-none p-1 rounded-sm hover:bg-white/5 transition-colors">
    ...
    <div className="relative">
      ...
      {/* Track */}
      <div
        className={`block w-10 h-5 rounded-sm transition-all duration-300 shadow-[inset_0_2px_4px_rgba(0,0,0,0.6),inset_0_0_2px_rgba(0,0,0,0.4),0_1px_0_rgba(255,255,255,0.05)] ${checked ? "bg-indigo-900/60" : "bg-zinc-900"}`}
      ></div>

      {/* Thumb */}
      <div
        className={`absolute left-0.5 top-0.5 w-4 h-4 rounded-sm transition-transform duration-300 ease-[cubic-bezier(0.175,0.885,0.32,1.275)]
...
export const CollapsibleSection: React.FC<SectionProps> = ({
  ...
  return (
    <div className="mb-2 bg-black/20 rounded-sm border border-white/5 overflow-hidden">
      ...
      >
        <span className="flex items-center gap-2 select-none drop-shadow-sm">
          {icon}
          {label}
        </span>
        <ChevronUp
