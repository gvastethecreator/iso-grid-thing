    ...
    >
      ...
      <div className="flex items-center">
        ...
        {onUndo && (
          <button
            onClick={onUndo}
            disabled={!canUndo}
            className={`${btnClass} ${!canUndo ? "opacity-30" : ""}`}
            aria-label="Undo"
          >
        ...
        {onRedo && (
          <button
            onClick={onRedo}
            disabled={!canRedo}
            className={`${btnClass} ${!canRedo ? "opacity-30" : ""}`}
            aria-label="Redo"
          >
        ...
        <button
          onClick={onReset}
          className={`${btnClass} rounded-r-md text-indigo-400 hover:text-indigo-300`}
      ...
      {!is2D && (
        <div className="flex items-center justify-between px-1.5 pb-1">
          <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-wider select-none">
            Camera Rotation
          </span>
